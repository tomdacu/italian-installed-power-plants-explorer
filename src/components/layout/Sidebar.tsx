...
function BackendStatus() {
  ...
  return (
    <div className="flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2">
      ...
      <span className="truncate text-xs font-medium text-ink-400">{state.label}</span>
    </div>
  );
}
    ...
    >
      {({ isActive }) => (
        <>
          <span
            className={cn(
              "absolute left-0 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-brand-400 transition-all duration-200",
              isActive ? "opacity-100" : "opacity-0",
            )}
          />
          <Icon className={cn("h-4 w-4 shrink-0 transition-colors", isActive ? "text-brand-400" : "group-hover:text-brand-300")} />
...
export function Sidebar() {
  ...
  return (
    <aside className="relative flex h-full w-[248px] shrink-0 flex-col overflow-hidden bg-forest-950">
      {/* Ambient brand glows */}
      ...
      <div className="relative flex items-center gap-3 px-5 pb-6 pt-6">
        ...
        <div className="min-w-0">
          <p className="font-display text-[15px] font-semibold leading-tight text-white">Italian Renewable</p>
      ...
      <div className="relative space-y-2 border-t border-white/[0.06] p-3">
        ...
        <button
          onClick={toggle}
          className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-sm font-medium text-ink-400 transition hover:bg-white/[0.05] hover:text-ink-100"
        >
